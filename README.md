...
Clone repo and setup the environment:
 ...
 ```
git clone https://github.com/2019ChenGong/Feta-Pro.git
cd Feta-Pro
bash install.sh
 ```
...
For baselines, readers can select the options `-m`: [`DP-NTK`, `DP-Kernel`, `DP-MERF`, `DPGAN`, `DPDM`, `GS-WGAN`, `DP-FETA`, `DP-FETA-Pro`], refering to implementations in [DPImageBench](https://github.com/2019ChenGong/DPImageBench).
```
...
For the results in Table 12, we can run `cal_privacy.py` to obtain the DP cost ratios of spatial feature / frequency feature / DP-SGD in FETA-Pro based on `train.sigma_freq`, `train.sigma_time`, and `epsilon`.
